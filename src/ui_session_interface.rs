...
impl<T: InvokeUiSession> Session<T> {
    ...
    pub fn printer_response(&self, id: i32, path: String, printer_name: String) {
        ...
        let to = std::env::temp_dir().join(format!("{}_printer_{id}", crate::get_app_name().to_lowercase()));
    ...
    if handler.is_port_forward() {
        if handler.is_rdp() {
            ...
            if handler.args.len() != 3
                ...
                || port <= 0
            {
                handler.on_error(&format!("Invalid arguments, usage:<br><br> {} --port-forward remote-id listen-port remote-host remote-port", crate::get_app_name().to_lowercase()));
            }
