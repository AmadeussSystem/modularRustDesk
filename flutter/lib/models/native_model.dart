...
class PlatformFFI {
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ...
        : isLinux
            ? DynamicLibrary.open('libapp.so')
            : isWindows
                ? DynamicLibrary.open('libapp.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibapp.dylib") :
  ...
  void _startListenEvent(appImpl appImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = appImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
