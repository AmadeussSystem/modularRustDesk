...
class _DesktopHomePageState extends State<DesktopHomePage>
  ...
  Widget buildHelpCards(String updateUrl) {
    if (!bind.isCustomClient() &&
        ...
        !isCardClosed &&
        bind.mainUriPrefixSync().contains('app')) {
      final isToUpdate = (isWindows || isMacOS) && bind.mainIsInstalled();
      String btnText = isToUpdate ? 'Update' : 'Download';
      GestureTapCallback onPressed = () async {
        final Uri url = Uri.parse('https://app.com/download');
        await launchUrl(url);
      };
      if (isToUpdate) {
        onPressed = () {
      ...
      return buildInstallCard(
          ...
          link: isToUpdate
              ? 'https://github.com/app/app/releases/tag/${bind.mainGetNewVersion()}'
              : null);
    }
    ...
    if (isWindows && !bind.isDisableInstallation()) {
      ...
      if (bind.isSelinuxEnforcing()) {
        ...
        if (bind.mainGetLocalOption(key: keyShowSelinuxHelpTip) != 'N') {
          LinuxCards.add(buildInstallCard(
            ...
            help: 'Help',
            link:
                'https://app.com/docs/en/client/linux/#permissions-issue',
            closeButton: true,
      ...
      if (bind.mainCurrentIsWayland()) {
        LinuxCards.add(buildInstallCard(
            ...
            marginTop: LinuxCards.isEmpty ? 20.0 : 5.0,
            help: 'Help',
            link: 'https://app.com/docs/en/client/linux/#x11-required'));
      } else if (bind.mainIsLoginWayland()) {
        LinuxCards.add(buildInstallCard("Warning",
            "Login screen using Wayland is not supported", "", () async {},
            marginTop: LinuxCards.isEmpty ? 20.0 : 5.0,
            help: 'Help',
            link: 'https://app.com/docs/en/client/linux/#login-screen'));
      }
  ...
  void initState() {
    ...
    appWinManager.registerActiveWindowListener(onActiveWindowChanged);
    ...
    appWinManager.setMethodHandler((call, fromWindowId) async {
      ...
      if (call.method == kWindowMainWindowOnTop) {
        ...
        await appWinManager.registerActiveWindow(call.arguments["id"]);
      } else if (call.method == kWindowEventHide) {
        await appWinManager.unregisterActiveWindow(call.arguments['id']);
        ...
        if (windowId != null && windowType != null) {
          await appWinManager.moveTabToNewWindow(
        ...
        await appWinManager.openMonitorSession(
            windowId, peerId, display, displayCount, screenRect, windowType);
      } else if (call.method == kWindowEventRemoteWindowCoords) {
        final windowId = int.tryParse(call.arguments);
        if (windowId != null) {
          return jsonEncode(
              await appWinManager.getOtherRemoteWindowCoords(windowId));
        }
      }
    });
